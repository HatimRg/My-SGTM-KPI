...
export default function QualifiedPersonnel() {
  ...
  useEffect(() => {
    const loadProjects = async () => {
      try {
        const res = projectService.getMyProjects ? await projectService.getMyProjects() : await projectService.getAll()
  ...
  useEffect(() => {
    fetchWorkers()
  }, [selectedProject, searchTerm, workersPage])
            ...
            <>
              <div className="card p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                ...
                <button
                  ...
                  className="btn-outline btn-sm flex items-center gap-2"
                  title={t('common.unselect') || 'Unselect'}
                >
                  <X className="w-4 h-4" />
