...
import { Mail, Lock, Eye, EyeOff, Loader2, Shield, CheckCircle2, Sparkles } from 'lucide-react'
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-gradient-to-br from-green-500 to-emerald-600">
        <div className="text-center text-white animate-bounce-in">
          <div className="relative">
            <div className="w-32 h-32 mx-auto mb-6 rounded-full bg-white/20 flex items-center justify-center animate-pulse-ring">
              <CheckCircle2 className="w-20 h-20 text-white animate-scale-in" />
            </div>
            <Sparkles className="absolute top-0 right-4 w-8 h-8 text-yellow-300 animate-sparkle" />
            <Sparkles className="absolute bottom-4 left-4 w-6 h-6 text-yellow-300 animate-sparkle-delayed" />
          </div>
          <h2 className="text-3xl font-bold mb-2 animate-slide-up">{t('auth.loginSuccess')}</h2>
          <p className="text-white/80 animate-slide-up-delayed">{t('auth.redirecting')}</p>
        </div>
      </div>
    )
  }
