...
export default function UserDashboard() {
  ...
  useEffect(() => {
    if (data && weekScrollRef.current) {
      ...
      if (year === currentWeekInfo.year) {
        const weekElement = weekScrollRef.current.querySelector(`[data-week="${currentWeekInfo.week}"]`)
        if (weekElement) {
          // Scroll to show current week 3rd from right
          const containerWidth = weekScrollRef.current.clientWidth
          const weekWidth = weekElement.offsetWidth + 8 // including gap
          const scrollPosition = weekElement.offsetLeft - containerWidth + (weekWidth * 3)
          weekScrollRef.current.scrollTo({ left: Math.max(0, scrollPosition), behavior: 'smooth' })
        }
      }
    }
  }, [data, year])
  ...
  const fetchDashboardData = async () => {
    try {
      setLoading(true)
      const response = await dashboardService.getUserDashboard(year)
