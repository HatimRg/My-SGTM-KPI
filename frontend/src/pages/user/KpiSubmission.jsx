    ...
    const loadExistingReport = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          ppe_compliance_rate: report.ppe_compliance_rate || 0, // 15. Conformité HSE
  ...
  const updateFormData = (field, value) => {
    setFormData(prev => {
      ...
      if (['accidents', 'lost_workdays', 'hours_worked'].includes(field)) {
        const hoursWorked = parseFloat(field === 'hours_worked' ? value : updated.hours_worked) || 0
        const accidents = parseFloat(field === 'accidents' ? value : updated.accidents) || 0
        const lostWorkdays = parseFloat(field === 'lost_workdays' ? value : updated.lost_workdays) || 0
              ...
              <DailyKpiEntry
                ...
                onDataConfirmed={(aggregates) => {
                  ...
                  setFormData(prev => {
                    ...
                    // TF (Frequency Rate) = (Accidents × 1,000,000) / Hours Worked
                    // TG (Severity Rate) = (Lost Workdays × 1,000) / Hours Worked
                    const hoursWorked = parseFloat(updated.hours_worked) || 0
                    const accidents = parseFloat(updated.accidents) || 0
                    const lostWorkdays = parseFloat(updated.lost_workdays) || 0
