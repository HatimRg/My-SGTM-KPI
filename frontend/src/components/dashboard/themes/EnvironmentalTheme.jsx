...
const EnvironmentalTheme = memo(function EnvironmentalTheme({ kpiSummary, weeklyTrends }) {
  ...
  const envMetrics = useMemo(() => ({
    waterConsumption: kpiSummary?.total_water_consumption || 0,
    electricityUsage: kpiSummary?.total_electricity_consumption || 0,
    hseCompliance: Math.round(kpiSummary?.avg_hse_compliance || 0),
    medicalCompliance: Math.round(kpiSummary?.avg_medical_compliance || 0),
    trainingHours: Math.round(kpiSummary?.total_training_hours || 0),
    nearMisses: kpiSummary?.total_near_misses || 0,
    workPermits: kpiSummary?.total_work_permits || 0
  }), [kpiSummary])
