...
export default function FilterTextInput({ label, value, onChange, className = '', inputClassName = '', ...rest }) {
  const id = rest.id || `filter-input-${String(label ?? 'field').toLowerCase().replace(/[^a-z0-9]+/g, '-')}`
  return (
