...
echo [3] Testing external connection...
powershell -Command "try { $response = Invoke-WebRequest -Uri 'http://16.171.8.116:8000/' -UseBasicParsing -TimeoutSec 5; Write-Host 'External: SUCCESS (Status:' $response.StatusCode ')' } catch { Write-Host 'External: FAILED' }"
echo.
...
set STARTUP_SHORTCUT=%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup\HSE-KPI-Tracker.larp
