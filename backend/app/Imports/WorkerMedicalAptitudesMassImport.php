...
class WorkerMedicalAptitudesMassImport implements ToCollection
{
    ...
    private function detectHeaderIndex(array $raw): ?int
    {
        ...
        for ($i = 0; $i < $max; $i++) {
            ...
            $hasAptitudeOrExam = false;
            foreach ($cells as $c) {
                if ($c === 'aptitude_status' || $c === 'exam_nature' || $c === 'exam_date' || str_contains($c, 'aptitude') || str_contains($c, 'exam')) {
                    $hasAptitudeOrExam = true;
                    break;
                }
            }

            if ($hasCinExact && $hasAptitudeOrExam) {
                return $i;
            }
        }
