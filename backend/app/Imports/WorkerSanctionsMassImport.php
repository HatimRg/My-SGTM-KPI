...
class WorkerSanctionsMassImport implements ToCollection
{
    ...
    private function detectHeaderIndex(array $raw): ?int
    {
        ...
        for ($i = 0; $i < $max; $i++) {
            ...
            $hasSanctionSignals = false;
            foreach ($cells as $c) {
                if ($c === 'date_sanction' || $c === 'type_sanction' || $c === 'reason' || str_contains($c, 'sanction') || str_contains($c, 'reason')) {
                    $hasSanctionSignals = true;
                    break;
                }
            }

            if ($hasCinExact && $hasSanctionSignals) {
                return $i;
            }
        }
