...
class WorkerSanctionsMassFailedRowsExport implements FromArray, WithColumnWidths, WithEvents, WithTitle
{
    private array $rows;
    private string $lang;
    ...
    {
        $out = [];
        $out[] = [$this->tr('SGTM - RAPPORT DES LIGNES EN ERREUR (SANCTIONS)', 'SGTM - FAILED ROWS REPORT (SANCTIONS)')];
        $out[] = [$this->tr(
        ...
        $out[] = $this->lang === 'en'
            ? ['#', 'CIN', 'DATE_SANCTION', 'TYPE_SANCTION', 'MISE_A_PIED_DAYS', 'REASON', 'ERROR']
            : ['#', 'CIN', 'Date sanction', 'Type sanction', 'Jours mise à pied', 'Motif', 'Erreur'];
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->getStyle("C{$dataStartRow}:C{$highestRow}")->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_DATE_DDMMYYYY);
            },
        ];
    }
}
