...
class WorkerQualificationsMassFailedRowsExport implements FromArray, WithColumnWidths, WithEvents, WithTitle
{
    private array $rows;
    private string $lang;
    ...
    {
        $out = [];
        $out[] = [$this->tr('SGTM - RAPPORT DES LIGNES EN ERREUR (QUALIFICATIONS)', 'SGTM - FAILED ROWS REPORT (QUALIFICATIONS)')];
        $out[] = [$this->tr(
        ...
        $out[] = $this->lang === 'en'
            ? ['#', 'CIN', 'TYPE_QUALIFICATION', 'NIVEAU_QUALIFICATION', 'LIBELLE_QUALIFICATION', 'DATE_DEBUT', 'DATE_EXPIRATION', 'ERROR']
            : ['#', 'CIN', 'Type qualification', 'Niveau qualification', 'Libellé qualification', 'Date début', 'Date expiration', 'Erreur'];
