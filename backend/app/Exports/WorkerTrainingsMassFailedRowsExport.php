...
class WorkerTrainingsMassFailedRowsExport implements FromArray, WithColumnWidths, WithEvents, WithTitle
{
    private array $rows;
    private string $lang;
    ...
    {
        $out = [];
        $out[] = [$this->tr('SGTM - RAPPORT DES LIGNES EN ERREUR (FORMATIONS)', 'SGTM - FAILED ROWS REPORT (TRAININGS)')];
        $out[] = [$this->tr(
        ...
        $out[] = $this->lang === 'en'
            ? ['#', 'CIN', 'TYPE_FORMATION', 'DATE_FORMATION', 'DATE_EXPIRATION', 'ERROR']
            : ['#', 'CIN', 'Type formation', 'Date formation', 'Date expiration', 'Erreur'];
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->getStyle("D{$dataStartRow}:E{$highestRow}")->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_DATE_DDMMYYYY);
            },
        ];
    }
}
